...
authenticator_class_map: Dict = {
    "Service": (GoogleServiceKeyAuthenticator, lambda credentials: json.loads(credentials["credentials_json"]))
}
...
class GoogleAnalyticsDataApiGenericStream(IncrementalGoogleAnalyticsDataApiStream):
    ...
    def read_records(
        ...
        if not stream_slice:
            return []
        records = super(GoogleAnalyticsDataApiGenericStream, self).read_records(
            sync_mode=sync_mode,
